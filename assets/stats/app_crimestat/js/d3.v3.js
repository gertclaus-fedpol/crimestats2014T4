(function() {
  ...
  /* Customization
   *
   * Description: addition of d3.i18n (internationalization), allowing the month / week / day names to be adapted from within the application
   * Author:      Haroen Vermylen (CSC)
   * Date:        26 Nov 2012
   */
  d3.i18n = {};
  d3.i18n.months = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  d3.i18n.days = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ];
 
  var d3_time = Date;
  var d3_time_days = function() { return d3.i18n.days; };
  var d3_time_daySymbols = d3_time_days();
  var d3_time_months = function() { return d3.i18n.months; };
  var d3_time_monthAbbreviations = function() { return d3_time_months().map(function(s) { return s.substring(0,3); }); };
  var d3_time_dayAbbreviations = function() { return d3_time_days().map(function(s) { return s.substring(0,3); }); };
  ...
  /* Customization
   *
   * Description: addition of %q and %Q options for quarter formatting
   * Author:      Haroen Vermylen (CSC)
   * Date:        04 Dec 2012
   */
    q: function(d) {
      var q = Math.ceil((d.getMonth() + 1) / 3);
      return (q == 1) ? "I" : ((q == 2) ? "II" : ((q == 3) ? "III" : "IV"));
    },
    Q: function(d) {
   ...
   * Date:        04 Dec 2012
   */
    q: d3_time_parseQuarterLatin,
    Q: d3_time_parseQuarter,
  /* End of customization */
    p: d3_time_parseAmPm,
   ...
   * Date:        04 Dec 2012
   */
  function d3_time_parseQuarter(date, string, i) {
    date.d = 1;
    var n = string.substring(i, i += 2).toLowerCase();
    var q = parseInt(n.substring(1,2));
    if (n.substring(0,1) == "q" && q >= 0 && q <= 4)
      return (date.m = ((q-1)*3), i);
    else
      return -1;
  }
  function d3_time_parseQuarterLatin(date, string, i) {
    date.d = 1;
    if (string.substring(i, i + 3) == "III") {
      date.m = 6; return (date, i+3);
    }
    else if (string.substring(i, i + 2) == "IV") {
      date.m = 9; return (date, i+2);
    }
    else if (string.substring(i, i + 2) == "II") {
      date.m = 3; return (date, i+2);
    }
    else if (string.substring(i, i + 1) == "I") {
      date.m = 0; return (date, i+1);
    }
    else {
      return -1;
    }
...
/**
 * Extend d3.selection with a number of convenience methods
 *
 * Author: CSC (KCA, HVE)
 * Date: 11 Sep 2012
 */ 
 
/** Get or set the current selected item in a combobox */
d3.selection.prototype.selected = function(arg) {
  if (arg !== undefined) {
    // Try to set the selection of the combobox
    this.selectAll("option").property("selected",function(d) { return d[1] == arg; });
    return this;
  }
  else {
    return this.selectAll("option").filter(function() { return d3.select(this).property("selected"); }).datum()[1];
  }
}

/** Show the elements in the selection */
d3.selection.prototype.show = function() {
  this.classed("hidden",false);
  return this;
}

/** Hide the elements in the selection */
d3.selection.prototype.hide = function() {
  this.classed("hidden",true);
  return this;
}

/** Toggle the elements in the selection */
d3.selection.prototype.toggle = function() {
  this.classed("hidden",!this.classed("hidden"));
  return this;
}

/** True if the elements in the selection are visible, false otherwise */
d3.selection.prototype.visible = function() {
  return !this.classed("hidden");
}

/** Return the actual width of the elements in the selection */
d3.selection.prototype.width = function() {
  return parseInt(this.style("width"),10); // Ignores 'px', 'em', ...
}

/** Return the actual height of the elements in the selection */
d3.selection.prototype.height = function() {
  return parseInt(this.style("height"),10); // Ignores 'px', 'em', ...
}

/** Set the color class to the elements in the selection */
d3.selection.prototype.color = function(colorClass) {
  if (this.visible()) {
